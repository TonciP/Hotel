...
export default function Cards() {
  ...
  const getTipoHabitacion = () => {
	/* fetch("https://localhost:7285/api/tipohabitacion") */
  fetch("https://localhost:7007/api/tipohabitacion")
  ...
  const filterCountry = (hotel) => {
    return (
      filter.country === "Todos los paises" || filter.country === hotel.country
    );
  };
    ...
    if (hotelList.length === 0) {
      return (
        <p className="errorMessage">
          No hay hoteles que coincidan con la búsqueda seleccionada. Por favor,
          ¡seguí buscando!
        </p>
      );
      ...
      return (
        <p className="errorMessage">
          Por favor seleccioná una fecha de check-out posterior a lafecha de
          check-in
        </p>
      );
    } else if (checkInDate < today) {
      return <DateErrorModal handleClose={handleCloseModal} open={open} />;
    } else
      /* return hotelList.map((hotel) => <Card {...hotel} key={hotel.slug} />); */
	  return tipoHabitacion.map((habitacion) => <Card {...habitacion} key={habitacion.habitacionId} />);
  };
